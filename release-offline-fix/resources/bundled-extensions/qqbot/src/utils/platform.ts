...
import { getQQBotRuntimeOptional } from "../runtime.js";
...
/**
 * 优先解析当前 Gateway 的状态目录。
 *
 * 多网关场景下必须跟随 OPENCLAW_STATE_DIR，避免不同 agent 的 QQBot
 * 共享同一份 session / downloads / known-users 数据。
 */
export function resolveOpenClawStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string | null {
...
function resolveQQBotRuntimeDataDir(): string | null {
  const runtime = getQQBotRuntimeOptional();
  if (!runtime || typeof runtime.getDataDir !== "function") {
    return null;
  }
  try {
...
/**
 * 获取 QQBot 插件的数据目录，并自动创建。
 *
 * 优先级:
 * 1. QQBOT_DATA_DIR（显式覆盖）
 * 2. PluginRuntime.getDataDir()（OpenClaw 运行时提供的插件隔离目录）
 * 3. OPENCLAW_STATE_DIR/qqbot
 * 4. CLAWDBOT_STATE_DIR/qqbot
 * 5. 测试环境临时目录
 * 6. ~/.openclaw/qqbot（兼容旧版本/独立运行）
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const explicitDataDir = process.env.QQBOT_DATA_DIR?.trim() || "";
  const runtimeDataDir = resolveQQBotRuntimeDataDir();
  const stateDir = resolveOpenClawStateDirFromEnv();
  const baseDir = explicitDataDir
    ? explicitDataDir
    : runtimeDataDir
      ? runtimeDataDir
      : stateDir
        ? path.join(stateDir, "qqbot")
        : process.env.VITEST || process.env.NODE_ENV === "test"
          ? path.join(os.tmpdir(), `openclaw-qqbot-${process.pid}`)
          : path.join(getHomeDir(), ".openclaw", "qqbot");
