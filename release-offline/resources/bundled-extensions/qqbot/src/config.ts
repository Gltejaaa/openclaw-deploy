...
import { resolveOpenClawStateDirFromEnv } from "./utils/platform.js";
...
function normalizeQQBotAccountConfig(input: unknown): QQBotAccountConfig {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return {};
  }

  const obj = input as Record<string, unknown>;
  const normalized = { ...(obj as QQBotAccountConfig) };
  ...
  const appSecret = typeof obj.appSecret === "string" ? obj.appSecret.trim() : "";
  const token = typeof obj.token === "string" ? obj.token.trim() : "";
  const tokenSecret = token.includes(":") ? token.slice(token.indexOf(":") + 1).trim() : "";

  if (appId) {
    normalized.appId = appId;
  }
  if (!normalized.clientSecret && (clientSecret || appSecret || tokenSecret)) {
    normalized.clientSecret = clientSecret || appSecret || tokenSecret;
  }

  return normalized;
}

function normalizeQQBotChannelConfig(input: unknown): QQBotChannelConfig | undefined {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return undefined;
  }

  const obj = input as Record<string, unknown>;
  const normalized = normalizeQQBotAccountConfig(obj) as QQBotChannelConfig;
  const accountsRaw = obj.accounts;

  if (accountsRaw && typeof accountsRaw === "object" && !Array.isArray(accountsRaw)) {
    const accounts: Record<string, QQBotAccountConfig> = {};
    for (const [accountId, value] of Object.entries(accountsRaw)) {
      accounts[accountId] = normalizeQQBotAccountConfig(value);
    }
...
function readLegacyQQBotChannelConfig(): QQBotChannelConfig | undefined {
  const stateDir = resolveOpenClawStateDirFromEnv();
  if (!stateDir) {
    return undefined;
  }

  const channelsPath = path.join(stateDir, "channels.json");
  try {
    ...
    return normalizeQQBotChannelConfig(parsed.qqbot ?? parsed.qq);
  } catch {
    return undefined;
  }
}

function getQQBotChannelConfig(cfg: OpenClawConfig): QQBotChannelConfig | undefined {
  const direct = normalizeQQBotChannelConfig(cfg.channels?.qqbot);
  const legacy = readLegacyQQBotChannelConfig();
  if (!legacy) {
    return direct;
  }
  if (!direct) {
  ...
  const merged: QQBotChannelConfig = {
    ...legacy,
    ...direct,
  };
  if (legacy.accounts || direct.accounts) {
    merged.accounts = {
      ...(legacy.accounts || {}),
      ...(direct.accounts || {}),
    };
...
export function listQQBotAccountIds(cfg: OpenClawConfig): string[] {
  ...
  const qqbot = getQQBotChannelConfig(cfg);
