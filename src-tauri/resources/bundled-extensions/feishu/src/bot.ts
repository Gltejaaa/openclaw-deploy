...
export function buildFeishuAgentBody(params: {
  ctx: Pick<
    FeishuMessageContext,
    "chatType" | "content" | "senderName" | "senderOpenId" | "mentionTargets" | "messageId" | "hasAnyMention"
  >;
  ...
  // Group sessions need a speaker prefix so the agent can distinguish members.
  // DMs already have one session per sender, so prefixing the body only pollutes the transcript.
