...
// 使用运行时解析的目录，避免模块加载时过早固定到全局路径
function getImageServerDir(): string {
  return process.env.QQBOT_IMAGE_SERVER_DIR || getQQBotDataDir("images");
}
  ...
  const connect = async () => {
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        // 组装消息体
        // 静态说明已经迁移到 skills；这里保持 BodyForAgent 只包含真实用户输入，
        // 避免 QQ 专属上下文被写进会话历史后又显示在聊天界面里。
        const channelSystemPrompt = account.systemPrompt?.trim() || "";
        ...
        const ctxPayload = pluginRuntime.channel.reply.finalizeInboundContext({
          ...
          ...(channelSystemPrompt ? { GroupSystemPrompt: channelSystemPrompt } : {}),
